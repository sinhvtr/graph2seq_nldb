...
class ModelGraph(object):
    def __init__(self, word_vocab, char_vocab, Edgelabel_vocab, options=None, mode='ce_train'):
        ...
        # self.encoder = G2S_encoder.GraphEncoder(
        ...
        elif options.attention_type == 'transformer':
            self.encoder_dim = options.d_model
            self.encoder_states = self.encoder.enc
        else:
        ...
        with variable_scope.variable_scope("generator"):
            ...
            # self.encoder_features = self.calculate_encoder_features(self.encoder_states, self.encoder_dim)
            # self.transformer_decoder = GraphDecoder(self.encoder_states, self.answer_ref, word_vocab, char_vocab,
            #                                         is_training, options)
            # self.accu, self.loss = self.transformer_decoder.train(self.answer_ref, word_vocab)

            # create generator
            self.generator = G2S_generator.CovCopyAttenGen(self, options, word_vocab)
            ...
            if mode == 'decode':
                ...
                self.accu, self.loss, _ = self.generator.transformer_train_mode(word_vocab, self.encoder_dim, self.encoder_states, self.encoder_features,
                    self.nodes, self.nodes_mask, self.init_decoder_state,
                    self.answer_inp, self.answer_ref, loss_weights, mode_gen='ce_loss')
                if mode == 'evaluate': return # not buiding training op for evaluation
            elif mode == 'rl_train':
