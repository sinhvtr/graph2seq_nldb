...
class GraphEncoder(object):
    def __init__(self, word_vocab=None, edge_label_vocab=None, char_vocab=None, is_training=True, options=None):
        ...
        ## Position encoding
        passage_node_representation += positional_encoding(passage_node_representation, options.max_answer_len)
        ## Position encoding
        ...
        with tf.variable_scope('graph_encoder'):
            ...
            for i in xrange(options.num_syntax_match_layer):
                ...
                ## Dropout
                self.enc = tf.layers.dropout(passage_node_hidden,
                ...
                for i in range(options.num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ...
                        ### Feed Forward
                        self.enc = ff(self.enc, num_units=[4 * options.d_model, options.d_model])
