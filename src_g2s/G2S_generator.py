...
from transformer_modules import positional_encoding, multihead_attention, ff
...
class CovCopyAttenGen:
    ...
    def transformer_train_mode(self, vocab, encoder_dim, encoder_states, encoder_features, node_idxs, node_mask,
            init_state, decoder_inputs, answer_batch, loss_weights, mode_gen='ce_loss'):
        '''
        encoder_dim: int-valued
        encoder_states: [batch_size, passage_len, encoder_dim], for calculating context vector
        ...
        ## Position encoding
        decoder_inputs_emb += positional_encoding(decoder_inputs_emb, options.max_answer_len)
        answer_batch_emb += positional_encoding(answer_batch_emb, options.max_answer_len)
        ## Position encoding

        ## Dropout
        decoder_inputs_emb = tf.nn.dropout(decoder_inputs_emb, (1 - options.dropout_rate))
        answer_batch_emb = tf.nn.dropout(answer_batch_emb, (1 - options.dropout_rate))
        ## Dropout
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            for i in range(options.max_answer_len):
                ...
                word_t = decoder_inputs_emb[i]  # the wordidx_t must from decoder_inputs for phrase model
                # word_t = self.embedding_lookup(wordidx_t)
                # if mode_gen in ('ce_loss', 'rl_loss',):
                #     wordidx_t = decoder_inputs[i]
                # word_t = self.embedding_lookup(wordidx_t)
