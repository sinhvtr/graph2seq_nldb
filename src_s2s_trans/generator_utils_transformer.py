...
class CovCopyAttenGen:
    ...
    def train_mode(self, vocab, encoder_dim, encoder_states, encoder_features, passage_word_idx, passage_mask,
        ...
        ## Position encoding
        decoder_inputs += positional_encoding(decoder_inputs, options.max_answer_len)
        answer_batch_unstack += positional_encoding(answer_batch_unstack, options.max_answer_len)
        ## Position encoding
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            for i in range(options.max_answer_len):
                wordidx_t = decoder_inputs[i] # the wordidx_t must from decoder_inputs for phrase model
                word_t = self.embedding_lookup(wordidx_t)
                ## Position encoding
                # word_t += positional_encoding(word_t, options.max_answer_len)
                ## Position encoding
                if i > 0:
