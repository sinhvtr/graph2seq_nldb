...
class ModelGraph(object):
    def __init__(self, enc_word_vocab=None, dec_word_vocab=None, char_vocab=None, POS_vocab=None, NER_vocab=None, options=None, mode='ce_train'):
        ...
        if options.two_sent_inputs: # take two sentences as inputs
            ...
            self.encoder = encoder_utils_trans.SeqEncoder(self, options,
                                word_vocab=enc_word_vocab, char_vocab=char_vocab, POS_vocab=POS_vocab, NER_vocab=NER_vocab)

        # encode the input instance
        self.encode_dim, self.encode_hiddens, self.init_decoder_state, self.memory = self.encoder.encode(is_training=is_training)
        ...
        with variable_scope.variable_scope("generator"):
            ...
            self.generator_trans = generator_utils_trans.CovCopyAttenGen(self, options, dec_word_vocab)
            # calculate encoder_features
            self.encoder_features = self.generator_trans.calculate_encoder_features(self.phrase_representations, self.encode_dim)
            ...
            if mode == 'decode':
                ...
                (self.state_t, self.context_t, self.coverage_t, self.attn_dist_t, self.p_gen_t, self.ouput_t,
                    self.topk_log_probs, self.topk_ids, self.greedy_prediction, self.multinomial_prediction) = self.generator_trans.decode_mode(
                ...
                _, _, self.greedy_words = self.generator_trans.train_mode(dec_word_vocab, self.encode_dim, self.phrase_representations, self.encoder_features,
                    self.phrase_idx, self.phrase_mask, self.init_decoder_state,
                    self.gen_input_words, self.in_answer_words, loss_weights, 'greedy', self.memory)
                # not buiding training op for this mode
                return
            elif mode == 'transformer':
                self.accu, self.loss, _ = self.generator_trans.train_mode(dec_word_vocab, self.encode_dim,
                                                                    self.phrase_representations, self.encoder_features,
                                                                    self.phrase_idx, self.phrase_mask,
                                                                    self.init_decoder_state,
                                                                    self.gen_input_words, self.in_answer_words,
                                                                    loss_weights, 'transformer', self.memory)
    ...
    def run_ce_training(self, sess, batch, options, only_eval=False):
        ...
        # print("Gen input words: ", batch.gen_input_words)
    ...
    def run_encoder(self, sess, batch, options, only_feed_dict=False):
        ...
        # print("In Passage Words: ", batch.sent1_word)
        if options.with_char:
